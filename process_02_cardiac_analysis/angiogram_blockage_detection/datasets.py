...
def tophat(img, fsize=20, return_img=True):
    """
    Applies Top-Hat and Black-Hat morphological transformations to enhance image contrast.
    """
    if not isinstance(img, np.ndarray):
        img = np.array(img).astype(np.float32)
    filterSize = (fsize, fsize)
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, filterSize)
    # White Top-Hat: Enhances bright objects against dark backgrounds
    wth = cv2.morphologyEx(img, cv2.MORPH_TOPHAT, kernel).astype(np.float32)
    # Black Top-Hat: Enhances dark objects against bright backgrounds
...
class ImageDataset(Dataset):
    """
    Standard dataset loader for image translation tasks.
    """

    def __init__(self, root, size=256, unaligned=False, mode='train', transform=None, return_img_name=False):
        if mode == 'train':
            self.transform = T.Compose([
                T.Lambda(lambda img: random_tophat(img, p=0.5)),  # Apply random Top-Hat enhancement
                T.RandomResizedCrop(size, scale=(0.6, 1), interpolation=Image.BICUBIC),
                T.RandomHorizontalFlip(p=0.5),
                T.Lambda(lambda x: random_rotate(x, p=1, angles=[0, 180])),  # Random Rotation
            ...
            self.transform = T.Compose([
                # T.Lambda(lambda img: tophat(img, 50)),  # Optional Top-Hat enhancement for testing
                T.Resize(size),
                T.ToTensor(),
                T.Normalize((0.5,), (0.5,))
            ])
...
class ImageDataset2(Dataset):
    """
    Dataset loader specifically for supervised fine-tuning of image segmentation.
    Ensures that identical transformations are applied to both the image and the mask.
    """
    ...
    def __getitem__(self, index):
        ...
        # Merge A and B into a multi-channel image to ensure synchronized transforms
